...
export default function UploadPanScreen({ navigation }) {
  const { driver, refreshDriver } = useAuth();
  const { theme } = useSettings();
  const [num, setNum] = useState(driver?.documents?.pan?.number || '');
  const [loading, setLoading] = useState(false);

  const handleUpload = async () => {
    if (!num.trim()) {
      Alert.alert('Details Required', 'Please enter your PAN Card details');
      return;
    }
    setLoading(true);
    try {
      await updateDocuments({
        documents: {
          pan: { number: num, status: 'pending' }
        }
      });
      await refreshDriver();
      Alert.alert('Success', 'PAN Card details updated successfully!');
      navigation.goBack();
    } catch (err) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { borderBottomColor: theme.border }]}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>Upload PAN</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        <View style={styles.camBox}>
          ...
          <Text style={[styles.camText, { color: theme.textSecondary }]}>Capture/Upload PAN Card Photo Proof</Text>
        </View>

        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          <Text style={[styles.label, { color: theme.textSecondary }]}>PAN CARD NUMBER</Text>
          <TextInput
