...
export default function UploadInsuranceScreen({ navigation }) {
  const { driver, refreshDriver } = useAuth();
  const { theme } = useSettings();
  const [num, setNum] = useState(driver?.documents?.insurance?.number || '');
  const [loading, setLoading] = useState(false);

  const handleUpload = async () => {
    if (!num.trim()) {
      Alert.alert('Details Required', 'Please enter your Vehicle Insurance details');
      return;
    }
    setLoading(true);
    try {
      await updateDocuments({
        documents: {
          insurance: { number: num, status: 'pending' }
        }
      });
      await refreshDriver();
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { borderBottomColor: theme.border }]}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>Upload Insurance</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        <View style={styles.camBox}>
          ...
          <Text style={[styles.camText, { color: theme.textSecondary }]}>Capture/Upload Vehicle Insurance Photo Proof</Text>
        </View>

        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          <Text style={[styles.label, { color: theme.textSecondary }]}>VEHICLE INSURANCE POLICY NUMBER</Text>
          <TextInput
            style={[styles.input, { color: theme.text, borderColor: theme.border }]}
            placeholder="POL-INS-9821882"
