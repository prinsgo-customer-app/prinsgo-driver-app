...
export default function UploadAadhaarScreen({ navigation }) {
  const { driver, refreshDriver } = useAuth();
  const { theme } = useSettings();
  const [num, setNum] = useState(driver?.documents?.aadhaar?.number || '');
  const [loading, setLoading] = useState(false);

  const handleUpload = async () => {
    if (!num.trim()) {
      Alert.alert('Details Required', 'Please enter your Aadhaar Card details');
      return;
    }
    setLoading(true);
    try {
      await updateDocuments({
        documents: {
          aadhaar: { number: num, status: 'pending' }
        }
      });
      await refreshDriver();
      Alert.alert('Success', 'Aadhaar Card details updated successfully!');
      navigation.goBack();
    } catch (err) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { borderBottomColor: theme.border }]}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>Upload Aadhaar</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        <View style={styles.camBox}>
          ...
          <Text style={[styles.camText, { color: theme.textSecondary }]}>Capture/Upload Aadhaar Card Photo Proof</Text>
        </View>

        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          <Text style={[styles.label, { color: theme.textSecondary }]}>AADHAAR CARD NUMBER</Text>
          <TextInput
            style={[styles.input, { color: theme.text, borderColor: theme.border }]}
            placeholder="8221 4421 9918"
